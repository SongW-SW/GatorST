...
def loader_construction(data_name, data_path, batch_size, k_neighbors=3,
    ...
    n_true_clusters = len(np.unique(y_all_true))
    n_clusters_test = n_true_clusters + 100

    X_all = PCA(n_components=200, random_state=42).fit_transform(X_all)
    print(f"PCA features shape: {X_all.shape}")

    K = n_pseudo_clusters
    soft_probs, y_pseudo, centroids = soft_kmeans(X_all, K)
    n_pseudo = K
    print(f"Soft K-means: {K} clusters, pseudo-label distribution: "
          f"{np.bincount(y_pseudo, minlength=K)}")

    from sklearn.neighbors import NearestNeighbors
    print(f"Building KNN graph on spatial locations (k={k_neighbors})...")
    ...
    return train_loader, val_loader, test_loader, input_dim, n_pseudo, n_clusters_test
